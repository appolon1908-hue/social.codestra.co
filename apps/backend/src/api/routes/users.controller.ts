...
export class UsersController {
  ...
  private getAccessPayload(req: Request): { sessionId: string } {
    return AuthChecker.verifyJWT(
  ...
  async rotateApiKey(
    ...
    const payload = this.getAccessPayload(req);
    await this._sessionService.requireRecentAuthentication(
      payload.sessionId,
      user.id
    );
  ...
  @Post('/sessions/recent-auth')
  async confirmRecentAuthentication(
    @GetUserFromRequest() user: User,
    @Req() req: Request,
    @Body('password') password: string
  ) {
    if (!(await this._userService.verifyPassword(user.id, password || ''))) {
      throw new HttpForbiddenException();
    }
    const payload = this.getAccessPayload(req);
    await this._sessionService.markRecentAuthentication(
      payload.sessionId,
      user.id
    );
    return { confirmed: true, validForSeconds: 600 };
  }
  ...
  async changePassword(
    ...
    if (
      !(await this._userService.verifyPassword(
        user.id,
        currentPassword || ''
      )) ||
    ...
    await this._userService.updatePasswordAndRevokeSessions(
      user.id,
      newPassword
    );
  ...
  async logout(
    ...
    const payload = AuthChecker.verifyJWT(
      (req.headers.auth as string) || req.cookies.auth
    ) as { sessionId: string };
    await this._sessionService.revoke(payload.sessionId, user.id);
  ...
  @Get('/sessions')
  listSessions(@GetUserFromRequest() user: User) {
    return this._sessionService.list(user.id);
  }
  ...
  async revokeAllSessions(
    ...
    await this._sessionService.revokeAll(user.id);
    return { revoked: true };
  }
