...
export class AuthService {
  ...
  async routeAuth(
    ...
    if (provider === Provider.LOCAL) {
      ...
      if (body instanceof CreateOrgUserDto) {
        ...
        const session = await this.session(create.users[0].user, ip, userAgent);
        const obj = { addedOrg, jwt: session.accessToken, ...session };
        const activationToken = AuthChecker.signPurposeToken(
          { id: create.users[0].user.id, activated: false, email: body.email },
          'account-activation',
          '24h'
        );
        await this._emailService.sendEmail(
          ...
          `Click <a href="${process.env.FRONTEND_URL}/auth/activate/${activationToken}">here</a> to activate your account`,
          'top'
        );
        return obj;
      }
      ...
      if (AuthChecker.passwordHashNeedsUpgrade(user.password)) {
        await this._userService.upgradePasswordHash(user.id, body.password);
      }
      ...
      const session = await this.session(user, ip, userAgent);
      return { addedOrg: false, jwt: session.accessToken, ...session };
    }
    ...
    const addedOrg =
      addToOrg && typeof addToOrg !== 'boolean'
        ...
        : false;
    const session = await this.session(user, ip, userAgent);
    return { addedOrg, jwt: session.accessToken, ...session };
  }
  ...
  async forgot(email: string) {
    ...
    const resetValues = AuthChecker.signPurposeToken(
      { id: user.id },
      'password-reset',
      '20m'
    );
  ...
  forgotReturn(body: ForgotReturnPasswordDto) {
    const user = AuthChecker.verifyPurposeToken(
      body.token,
      'password-reset'
    ) as { id: string };
    return this._userService.updatePasswordAndRevokeSessions(
      user.id,
      body.password
    );
  }
  ...
  async activate(
    ...
    const user = AuthChecker.verifyPurposeToken(code, 'account-activation') as {
      id: string;
      activated: boolean;
      email: string;
    };
    if (user.id && !user.activated) {
      ...
      return this.session(user as any, ip, userAgent);
    }
  ...
  async resendActivationEmail(email: string) {
    ...
    const jwt = AuthChecker.signPurposeToken(
      { id: user.id, activated: user.activated, email: user.email },
      'account-activation',
      '24h'
    );
  ...
  async checkExists(
    ...
    if (checkExists) {
      const session = await this.session(checkExists, ip, userAgent);
      return { jwt: session.accessToken, ...session };
    }

    return { token };
  }

  async session(user: User, ip: string, userAgent: string) {
    const organizations = await this._organizationService.getOrgsByUserId(
      user.id
    );
    const organization = organizations.find((item) => !item.users[0]?.disabled);
    if (!organization) throw new Error('No active organization');
    return this._sessionService.create(user.id, organization.id, ip, userAgent);
  }
}
