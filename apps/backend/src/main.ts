...
async function start() {
  ...
  const trustedProxyIps = (process.env.TRUSTED_PROXY_IPS || '')
    ...
    .filter(Boolean);
  if (!process.env.NOT_SECURED && trustedProxyIps.length === 0) {
    throw new Error(
      'TRUSTED_PROXY_IPS must name the exact Caddy proxy addresses'
    );
  }
  app
    .getHttpAdapter()
    ...
    .set(
      'trust proxy',
      (ip: string) =>
        ip === '127.0.0.1' || ip === '::1' || trustedProxyIps.includes(ip)
    );
  ...
  app.use(['/copilot/{*splat}', '/posts'], (req: any, res: any, next: any) => {
    json({ limit: process.env.AGENT_JSON_LIMIT || '2mb' })(req, res, next);
  });
