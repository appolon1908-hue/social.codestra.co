...
const getMcpConfig = (
  ...
  if (method === 'path') {
    switch (client) {
      case 'Claude Code':
        return {
          config: `claude mcp add codestra --transport http "${urlWithKey}"`,
          hint: 'Run this command in your terminal.',
        };
      case 'Cursor':
        return {
          config: json({ mcpServers: { codestra: { url: urlWithKey } } }),
          hint: 'Add to .cursor/mcp.json in your project root.',
        };
      case 'VS Code / Copilot':
        return {
          config: json({
            servers: { codestra: { type: 'http', url: urlWithKey } },
          }),
      ...
      case 'Windsurf':
        return {
          config: json({
            mcpServers: { codestra: { serverUrl: urlWithKey } },
          }),
          hint: 'Add to ~/.codeium/windsurf/mcp_config.json',
        };
      case 'Amp':
        return {
          config: `amp mcp add codestra ${urlWithKey}`,
          hint: 'Run this command in your terminal.',
        };
      case 'Codex':
        return {
          config: `# ~/.codex/config.toml\n\n[mcp_servers.codestra]\nurl = "${urlWithKey}"`,
      ...
      case 'Warp':
        return {
          config: json({ codestra: { url: urlWithKey } }),
  ...
  switch (client) {
    case 'Claude Code':
      return {
        config: `claude mcp add --transport http codestra ${urlBase} --header "Authorization: ${bearer}"`,
        hint: 'Run this command in your terminal.',
      };
    case 'Cursor':
      return {
        config: json({
          mcpServers: {
            codestra: { url: urlBase, headers: { Authorization: bearer } },
          },
        }),
        hint: 'Add to .cursor/mcp.json in your project root.',
      };
        ...
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              ...
              'Connect Codestra MCP server to your client (Http streaming) to schedule your posts faster.'
            )}
          </div>
        </div>
...
const localCliSteps = [
  ...
  {
    label: 'Install the Codestra skill for your AI agent',
      ...
      <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              'cli_description',
              'Use the Codestra CLI to automate posting from your terminal, or install the skill to let your AI agent schedule posts for you.'
            )}
          </div>
        </div>
        <div className="flex gap-[6px] shrink-0 pt-[2px]">
...
const PublicApiContent = () => {
  ...
  const rotateKey = useCallback(async () => {
    ...
    if (!approved) return;
    const recentAuth = await fetch('/user/sessions/recent-auth', {
      method: 'POST',
      body: JSON.stringify({ password: recentAuthPassword }),
    });
    setRecentAuthPassword('');
    if (!recentAuth.ok) {
      toaster.show(
        t('recent_auth_failed', 'Recent authentication failed'),
        'warning'
      );
      return;
    }
    ...
    const created = (await response.json()) as { apiKey?: string };
  ...
  }, [decision, fetch, mutate, recentAuthPassword, t, toaster]);

  if (!user) {
    return null;
  }

  const apiKey = oneTimeApiKey;

  if (!apiKey) {
    return (
      <div className="rounded-[12px] border border-newBorder bg-newBgColorInner p-[20px]">
        <div className="text-[15px] font-[600]">{t('api_key', 'API Key')}</div>
        <p className="mt-[8px] text-[13px] text-customColor18">
          {t(
            'api_key_one_time_notice',
            'API keys are displayed once and cannot be revealed later. Confirm recent authentication, then create a new key and store it securely.'
          )}
        </p>
        <label
          className="mt-[16px] block text-[13px] font-[600]"
          htmlFor="api-key-recent-password"
        >
          {t('confirm_password', 'Confirm password')}
        </label>
        <input
          id="api-key-recent-password"
          type="password"
          autoComplete="current-password"
          value={recentAuthPassword}
          onChange={(event) => setRecentAuthPassword(event.target.value)}
          className="mt-[6px] min-h-[44px] w-full max-w-[420px] rounded-[8px] border border-newBorder bg-newBgColorInnerInner px-[12px]"
        />
        <button
          type="button"
          onClick={rotateKey}
          disabled={!recentAuthPassword}
          className="mt-[16px] min-h-[44px] rounded-[8px] bg-[#612BD3] px-[16px] text-[13px] font-[600] text-white"
        >
  ...
  return (
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        ...
        {t(
          'api_auth_note_line2',
          'If you are building a product that schedules posts on behalf of other Codestra users,'
        )}
        <br />
        {t(
          'api_auth_note_line3',
        ...
        {t(
          'api_auth_note_line4',
          'and you will receive a cds_ prefixed token that works with the API, MCP, and CLI — just like an API Key.'
        )}
      </div>
      <div className="bg-newBgColorInnerInner rounded-[12px] border border-newBorder overflow-hidden">
        <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
          <div>
            ...
            <div className="text-[13px] text-customColor18 mt-[2px]">
              {t(
                ...
                'Use Codestra API to integrate with your tools.'
              )}
            </div>
          </div>
          <div className="flex gap-[6px] shrink-0 pt-[2px]">
        ...
        <div className="p-[20px] flex flex-col gap-[16px]">
          ...
          <label
            ...
            htmlFor="api-key-rotate-password"
          >
            {t(
              'confirm_password_to_rotate',
              'Confirm password to rotate this key'
            )}
          </label>
          <input
            id="api-key-rotate-password"
            type="password"
            autoComplete="current-password"
            value={recentAuthPassword}
            onChange={(event) => setRecentAuthPassword(event.target.value)}
            className="min-h-[44px] w-full max-w-[420px] rounded-[8px] border border-newBorder bg-newBgColorInnerInner px-[12px]"
          />
          <div className="flex gap-[8px]">
            <button
              type="button"
      ...
      <McpSection user={{ publicApi: apiKey }} mcpBase={mcpBase} />
    </div>
  );
};
