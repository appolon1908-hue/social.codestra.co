...
export const Agent: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { aiAgentEnabled } = useVariables();
  ...
  if (!aiAgentEnabled) {
    return (
      <main
        ...
        role="status"
      >
        <section className="max-w-lg rounded-2xl border border-white/10 bg-newBgColorInner p-8 text-center">
          <h1 className="mb-3 text-2xl font-semibold">Codestra AI Agent</h1>
          <p className="text-textColor">
            AI Agent is not configured. Contact your Codestra administrator to
            enable it.
          </p>
        </section>
      </main>
