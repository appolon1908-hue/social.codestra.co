...
def _backup_tools(root: Path) -> Path:
    ...
    _executable(
        ...
        'if [ "$count" = 0 ]; then echo social_test; else echo 0; fi\n'
        'echo $((count + 1)) >"$state"\n',
    )
    _executable(
        tools / "pg_dump",
    ...
    _executable(
        tools / "gpg",
        'out=\nwhile [ "$#" -gt 0 ]; do\n'
        '  case "$1" in --output) out=$2; shift 2;; --recipient) shift 2;; *) shift;; esac\n'
        'done\n: "${out:?}"\ncat >"$out"\n',
    )
...
class SocialRecoveryAuthorityTests(unittest.TestCase):
    ...
    def test_restore_is_empty_isolated_tuple_bound_and_verifying(self) -> None:
        for required in (
            ...
            "isolated restore database is not empty", "isolated uploads target is not empty",
    ...
    def test_freshness_verifies_backup_checksum_and_age(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (artifact / "METADATA").write_text("sanitized fixture")
    ...
    def test_backup_behavior_publishes_only_encrypted_atomic_pair(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                ["METADATA", "SHA256SUMS", "recovery.tar.gpg"],
            )
