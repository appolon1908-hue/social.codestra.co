...
export class NostrProvider extends SocialAbstract implements SocialProvider {
  ...
  async authenticate(params: {
    ...
    try {
      ...
      const protectedPrivateKey = encryptSecret(
        body.password,
        `nostr:${pubkey}:private-key`
      );
      return {
        id: pubkey,
        name: user.display_name || user.displayName || user.name || 'No Name',
        accessToken: `enc1:${Buffer.from(
          JSON.stringify(protectedPrivateKey)
        ).toString('base64url')}`,
        refreshToken: '',
  ...
  private privateKey(id: string, accessToken: string): string {
    if (accessToken.startsWith('enc1:')) {
      const encrypted = JSON.parse(
        Buffer.from(accessToken.slice(5), 'base64url').toString('utf8')
      ) as EncryptedValue;
      return decryptSecret(encrypted, `nostr:${id}:private-key`);
    }
    return (AuthService.verifyJWT(accessToken) as { password: string })
      .password;
  }

  private privateKeyBytes(id: string, accessToken: string): Uint8Array {
    const value = this.privateKey(id, accessToken);
    const pairs = value.match(/.{1,2}/g);
    if (!pairs || pairs.length !== 32)
      throw new Error('invalid_nostr_private_key');
  ...
  async post(
    ...
    const password = this.privateKeyBytes(id, accessToken);
