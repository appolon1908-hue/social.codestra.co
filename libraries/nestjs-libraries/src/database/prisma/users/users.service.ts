...
export class UsersService {
  ...
  async verifyPassword(id: string, password: string) {
    ...
    return (
      !!user?.password && AuthChecker.comparePassword(password, user.password)
    );
  }
      ...
      await Promise.all(
        [kept, switched].map((account) =>
          this._notificationService
            .sendEmail(
              account.email,
              'Your Codestra login was changed',
              `An administrator changed the login for your Codestra account. ` +
  ...
  upgradePasswordHash(id: string, password: string) {
    return this._usersRepository.upgradePasswordHash(id, password);
  }

  async updatePasswordAndRevokeSessions(id: string, password: string) {
    const updated = await this._usersRepository.updatePassword(id, password);
    await this._sessionService.revokeAll(id, 'password_changed', false);
    return updated;
  }
